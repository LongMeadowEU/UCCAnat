...
if(logged_in_imports() !== true) {
	...
	if($radio == "speciment_ref_num") {
		$result = mysql_query("SELECT * FROM import_specimens WHERE specimen_reference_number LIKE '%$search%'") or die($connect_error1);
	} else if($radio == "specimen_item_num") {
		$result = mysql_query("SELECT * FROM import_specimens WHERE specimen_item_number LIKE '%$search%'") or die($connect_error1);
	} else if($radio == "import_delivery_date") {
		
		$result = mysql_query("SELECT * FROM import_specimens WHERE specimen_delivery_date LIKE '%$search%'") or die($connect_error1);
	}
