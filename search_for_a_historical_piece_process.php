...
$connect_error1 = 'Data Base Connection Issue - histPieces';
$search = $_POST['search'];
$radio = $_POST['optionsRadiosInline'];

$show_form_new = true;
$histPieceRefQ = "SELECT * FROM historical_pieces_table WHERE donor_reference_num_hist_pieces LIKE '%$search%'";
$histPiecetypeQ = "SELECT * FROM historical_pieces_table WHERE type_of_specimen	LIKE '%$search%' OR other_specimen_type LIKE '%$search%'";
$histPieceDatedispQ = "SELECT * FROM historical_pieces_table WHERE date_disposed_of LIKE '%$search%'";
$histPieceAquiredQ = "SELECT * FROM historical_pieces_table WHERE date_aquired_on LIKE '%$search%'";
if ($radio == "hist_piece_ref_num") {
    $result = mysqli_query($db_connect, $histPieceRefQ) or die($connect_error1);
} else if ($radio == "hist_piece_type") {
    $result = mysqli_query($db_connect, $histPiecetypeQ) or die($connect_error1);
} else if ($radio == "hist_disposal_date") {
    $result = mysqli_query($db_connect, $histPieceDatedispQ) or die($connect_error1);
} else if ($radio == "hist_aquired_date") {
    $result = mysqli_query($db_connect, $histPieceAquiredQ) or die($connect_error1);
}
