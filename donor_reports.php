    ...
    <body>
        ...
        <div id="wrapper">
            ...
            if (empty($_POST) === false) {
                if ($_POST['report_year_search_box']) {
                    ...
                    $result_year = mysqli_query($db_connect, "SELECT * FROM  donor_table WHERE YEAR(date_of_receipt) LIKE '%$searched_year%' ") or die($connect_error1);
                    ...
                    <div class="row" id="welcomePageMainDiv">
                        <div class="col-lg-10">
                            ...
                            <ol class="breadcrumb">
                                ...
                                <li class="active">
                                    <a href="donor_reports.php"><strong>Donors Received in Year</strong></a>
                                </li>
                            </ol>
                        </div><!-- /.col-lg-10 -->
                    </div><!-- /.row -->
                                                ...
                                                <table class="table table-striped table-bordered table-hover sortable" id="donor_reports_table">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        while ($record = mysql_fetch_assoc($result_year)) {
                                                            ...
                                                            $date_of_death_formatted = date('d-m-Y', strtotime($date_of_death));

                                                            $date_of_receipt = $record['date_of_receipt'];
                                                            $date_of_receipt_formatted = date('d-m-Y', strtotime($date_of_receipt));

                                                            echo '<tr>';
                                                            echo '<td>' . ucwords($record['first_name']) . ' ' . ucwords($record['sur_name']) . '</td>';
                                                            echo '<td>' . $dob_formatted . '</td>';
                                                            echo '<td>' . $record['sex'] . '</td>';
                                                            echo '<td>' . ucwords($record['religion']) . '</td>';
                                                            echo '<td>' . $record['cause_of_death'] . '</td>';
                                                            echo '<td>' . $date_of_death_formatted . '</td>';
                                                            echo '<td>' . ucwords($record['place_of_death']) . '</td>';
                                                            echo '<td>' . $date_of_receipt_formatted . '</td>';
                                                            echo '</tr>';
                                                        }
                                                        ?>
                                                    </tbody>
                                ...
                                <div class="col-lg-4 col-md-offset-4">
                                    <form role="form" id="search_prosections" action="" method="POST">
                                        <div class="input-group custom-search-form" style="margin-top:15%">
                                            <input type="number" id="report_year_search_box" name="report_year_search_box" class="form-control" placeholder="Enter Year to Search - Donors Received">
        ...
        <script>
                                                ...
                                                function new_report_search() {
                                                    window.location = "donor_reports.php";
                                                }
